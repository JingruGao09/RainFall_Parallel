...
class Landscape {
  ...
  vector<vector<double> > land_drops; // matrix to record current drops value on each point
  vector<vector<double> > update_matrix; // matrix to record the value that needed to update to each point
  vector<vector<double> > land_absorb; // matrix to record the value of obsorbtion for each point
